...
def execute_mock_trade(symbol: str, action: str, price: float, confidence: float) -> Dict:
    ...
    if action.lower() == "buy":
        ...
        if cost <= usd_balance and amount > 0:
            ...
            new_entry_price = (
                (holding["entry_price"] * holding["amount"]) + (price * amount)
            ) / total_amount if holding["amount"] > 0 else price
...
def execute_mother_ai_decision(decision_data: Dict) -> List[Dict]:
    ...
    if isinstance(decisions, dict):
        decisions = [decisions]  # wrap single decision

    if not decisions:
        logger.warning("Mother AI returned no decisions.")
        return []

    logger.info(f"🧠 Executing {len(decisions)} Mother AI trade(s)...")
    trade_results = []
    ...
    for d in decisions:
        ...
        if symbol and action and price:
            ...
            logger.warning(f"⚠️ Invalid trade decision skipped: {d}")
