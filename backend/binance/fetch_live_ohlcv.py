...
def fetch_ohlcv(symbol: str, interval="1h", limit=100) -> pd.DataFrame:
    """
    Fetch live OHLCV data with current price integration.
    Default interval is now 1h which is perfect for your use case.
    """
    try:
        ...
        # 3. Modify the last candle's close/high/low with live price
        if not df.empty:
        ...
        print(f"✅ [Live OHLCV] Returning {len(df)} 1h candles with live-updated close")
